...
%type <node_uuid> StringConst FuncRPAREms MulExp MulOp AddExp AddOp RelExp RelOp EqExp EqOp LAndExp LOrExp
    ...
    | LBRACE RBRACE
        {
            ObjTree::ObjUuid uuid = obj_manager->get_uuid();
            obj_manager->set_type(uuid, "InitValAst");
            obj_manager->set_info(uuid, "InitValList");
            $$ = uuid;
        }
;
...
StringConst
    : STRING_LITERAL
        {
            ObjTree::ObjUuid uuid = obj_manager->get_uuid();
            obj_manager->set_type(uuid, "StringConstAst");
            obj_manager->set_info(uuid, $1);
            $$ = uuid;
        }
;
