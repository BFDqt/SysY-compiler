...
src/frontend/compiler2022_x/FrontEnd/YaccLex/src/sysy.l
                              当前 CMake 使用的 Flex 词法规则
src/frontend/compiler2022_x/FrontEnd/YaccLex/src/sysy.y
                              当前 CMake 使用的 Bison 语法规则
src/frontend/driver.cpp       ObjTree 到本项目 AST 的适配层
src/frontend/sysy.l           项目早期自写 Flex 词法规则，当前未被 CMake 使用
src/frontend/sysy.y           项目早期自写 Bison 语法规则，当前未被 CMake 使用
...
src/frontend/compiler2022_x/FrontEnd/YaccLex/src/sysy.y  871 行
src/frontend/compiler2022_x/FrontEnd/YaccLex/src/sysy.l  111 行
src/frontend/sysy.y                 642 行
src/frontend/sysy.l                 105 行
```

按当前实际参与构建的代码链路统计，不计 `tests`、`docs`、`build`、`temp` 以及早期保留但当前未被 CMake 使用的 `src/frontend/sysy.l`、`src/frontend/sysy.y`，主要模块代码量占比如下：

| 模块 | 统计范围 | 行数 | 占比 |
| --- | --- | ---: | ---: |
| 前端与 ObjTree 适配 | `src/frontend/driver.cpp`、`src/frontend/compiler2022_x` 中参与构建的词法、语法、Parser、ObjTree 和字符串工具 | 1751 | 38.3% |
...
另外，项目中还保留了早期自写前端 `src/frontend/sysy.y` 和 `src/frontend/sysy.l`，合计 747 行。若把这部分历史实现也计入仓库实现代码，前端相关代码总量为 2498 行，仓库主要实现代码总量为 5315 行，前端占比约为 47.0%。这也反映出本项目在前端接口迁移和树结构适配上保留了较多过程性代码。
...
项目中保留了两套前端材料。早期实现位于 `src/frontend/sysy.l` 和 `src/frontend/sysy.y`，直接在 Bison 动作中构造本项目的 AST。后期集成阶段又引入了 `src/frontend/compiler2022_x` 目录中的前端，该前端先生成 ObjTree 形式的语法树，再由 `src/frontend/driver.cpp` 中的 `ObjTreeAdapter` 转换为本项目的 AST。当前 `CMakeLists.txt` 实际使用的是 `compiler2022_x/FrontEnd/YaccLex/src/sysy.l` 和 `compiler2022_x/FrontEnd/YaccLex/src/sysy.y`。

当前参与构建的词法分析器位于 `src/frontend/compiler2022_x/FrontEnd/YaccLex/src/sysy.l`。它按 SysY2022 语言定义整理 token 类别，再把词素字符串通过 `yylval.raw_string` 传给 Bison 侧的 ObjTree 构造动作。

已实现的词法类别包括：

- 关键字：`const`、`int`、`float`、`void`、`if`、`else`、`while`、`break`、`continue`、`return`。
- 标识符：以字母或下划线开头，后接字母、数字或下划线。
- 整型常量：支持十进制、八进制和十六进制，适配层转换 AST 时使用 `std::strtol(raw.c_str(), nullptr, 0)`。
- 浮点常量：支持十进制浮点和十六进制浮点，适配层转换 AST 时使用 `std::strtof`。
- 字符串字面量：先保存原始字符串，转换为本项目 AST 时调用 `decodeSysYString` 处理转义。
- 运算符和分隔符：包括算术、关系、逻辑、赋值、括号、方括号、大括号、逗号和分号。
- 注释和空白：跳过单行注释、多行注释、空格、制表符和换行。

当前 `compiler2022_x` 词法器维护了 `line_number`，多行注释使用 Flex start condition `C_COMMENT` 处理，遇到 EOF 仍处在注释状态时抛出 `unterminated block comment`。早期自写词法器则维护 `yylineno` 和 `yycolumn`，并通过 `YY_USER_ACTION` 写入 token 位置。

词法阶段也处理了两个容易遗漏的错误：未闭合的块注释和未闭合的字符串字面量。遇到这些情况时直接抛出 `CompileError`，错误信息带源码行列号。

### 4.2 语法分析

当前语法分析器位于 `src/frontend/compiler2022_x/FrontEnd/YaccLex/src/sysy.y`。它的 Bison 动作不直接构造本项目 AST，而是向 `ObjTree::ObjManager` 申请节点，设置节点类型、节点信息和子节点列表。典型节点名包括 `CompUnitAst`、`DeclAst`、`FuncDefAst`、`StmtAst`、`LValAst`、`LAndExpAst`、`LOrExpAst` 等。

`src/frontend/driver.cpp` 中的 `ObjTreeAdapter` 负责把 ObjTree 转换成本项目 AST。适配层按照节点名分派：
...
早期自写语法器 `src/frontend/sysy.y` 则直接在 Bison 动作中构造本项目 AST。两套实现的语义出口保持一致，后续语义分析只看到 `include/sysy/ast.hpp` 中定义的 AST。
...
第一，顶层 `BType IDENT` 既可能是变量定义，也可能是函数定义。早期自写前端把函数形式 `BType IDENT '(' ... ')' Block` 和变量形式 `BType IDENT Dimensions VarInitOpt VarDefSuffixList ';'` 分开写，避免在语义阶段再猜测节点含义。当前 `compiler2022_x` 前端还额外在词法阶段区分了 `KEYWORD_INT_FUNC`、`KEYWORD_FLOAT_FUNC` 这类函数返回类型 token，用于辅助顶层函数定义规约。

第二，`if-else` 的悬挂 else 问题在早期自写前端中通过 Bison 优先级解决。当前 `compiler2022_x` 前端仍保留了相关规约冲突，Docker 构建中报告的 shift/reduce 冲突很可能就包含这类问题；现有测试可以通过，但这不是最理想的语法状态。

第三，Flex/Bison 的 C 风格接口和 C++ AST 所有权需要衔接。早期自写语法动作使用 `std::unique_ptr` 作为 AST 的实际所有权模型，但 Bison `%union` 只能直接传裸指针，所以实现了 `own`、`takeVector`、`takeString` 等小工具：Bison 动作创建节点时临时使用裸指针，规约完成后立刻转入 `unique_ptr` 或 `std::vector`，字符串 token 用完后释放 `malloc` 得到的内存。当前 `compiler2022_x` 前端先把节点交给 `ObjManager` 管理，转换成本项目 AST 时再进入 `unique_ptr` 所有权模型。

第四，集成 `compiler2022_x` 后需要做节点模型转换。ObjTree 中只有字符串形式的节点类型和节点信息，本项目语义分析需要的是强类型 AST，因此 `ObjTreeAdapter` 中写了较多结构校验逻辑，例如 `expect`、`onlySon`、`flattenLeftList`。如果节点形态和预期不一致，适配器会抛出 `compiler2022 frontend adapter` 前缀的 `CompileError`，方便区分是语义错误还是前端适配错误。
...
所有节点结构都带 `SourceLocation`，语义阶段的错误类型也统一通过该字段输出。需要注意的是，当前 ObjTree 适配层暂时把位置固定为 `1:1`，所以最终错误位置还没有完全发挥这套字段的作用；早期自写前端可以提供更精确的行列号。
...
第一，早期自写 Flex 前端中，`YY_USER_ACTION` 宏展开后需要能作为语句使用。实现中最后保留了 `do { ... } while (0);` 的形式，否则生成的 lexer C++ 代码会出现 `expected ';' after do/while statement` 之类错误。
...
第三，Bison 顶层规则如果直接按 EBNF 粗略翻译，`BType IDENT` 之后的分支容易混淆函数定义和变量声明。早期自写语法文件把函数顶层项和变量顶层项拆开，并让变量顶层项先读取第一个 `IDENT` 的维度和初始化，再接后续逗号定义。当前 `compiler2022_x` 语法器在 Docker 构建时仍报告 2 个 shift/reduce 冲突和 2 个 reduce/reduce 冲突，同时提示 `KEYWORD_INT`、`KEYWORD_FLOAT` 两条规则因冲突而无用。虽然现有测试可以通过，但这说明前端语法仍有可清理空间。

第四，ObjTree 适配层当前没有把真实 token 位置传到本项目 AST。`ObjTreeAdapter` 使用固定的 `SourceLocation(1, 1)`，所以当前前端下的语义错误都能报出错误类型，但行列号会退化为 `1:1`。早期自写前端能提供更精确的行列号，这一点需要在后续集成时补回。
...
同时构建中出现以下警告：`compiler2022_x` 的 Bison 语法有 2 个 shift/reduce 冲突、2 个 reduce/reduce 冲突；`ObjManager.h` 使用了 C99 compound literal；`ErrorMessage.h` 中有未使用参数和未使用函数。这些警告没有阻止构建，但应作为后续清理项记录。
...
计算结果为 `1 - 6 + 2 = -3`。在 `docs/ai_codegen.md` 的旧验证摘录中记录为退出码 253，按 Linux 退出码规则正好对应返回值 -3。报告撰写时也重新核对了源码表达式，确认这条记录与样例语义一致。
...
负向样例的错误类型均正确，但当前 ObjTree 适配层没有保留真实源码位置，因此行列号统一显示为 `1:1`。`docs/ai_codegen.md` 中较早的自写前端验证记录里，这几条错误曾带有更精确的 `3:7`、`3:3`、`2:3`、`6:10` 等位置。当前报告按最终源码复核结果记录为 `1:1`。
...
第五，当前 `compiler2022_x` 前端适配能通过现有样例，但构建仍有 Bison 冲突和位置退化问题。后续应优先消除语法冲突，并把词法位置信息一路传递到 ObjTree 和本项目 AST。
